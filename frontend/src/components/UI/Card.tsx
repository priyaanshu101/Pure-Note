...
export const Card = (props : CardProps & {onClickDelete?: () => void}) => {
    ...
    return (
        <div className="p-4 bg-white rounded-lg shadow-md border-slate-200 max-w-80 border font-semibold hover:bg-primary-200 shadow-lg hover:scale-105 transition-all duration-300 ease-out will-change-transform px-4 py-2 rounded-xl">
            <div className={`${CardHeaderStyle}`}>{props.startIcon} {props.title}
                <div className="flex">
                    {<RedirectIcon size="md"/>} {<div className="hover:bg-red"><DeleteIcon size="md" onClick={props.onClickDelete}/></div>}
                </div>
            </div>
            ...
            {props.type === "Twitter" && <blockquote className="twitter-tweet" data-width="100%">
                <a href={`${props.link}`}></a>
            </blockquote>}
