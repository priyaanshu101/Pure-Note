...
function UserBoard() {
  ...
  const [shareContentType, setShareContentType] = useState<'public' | 'private'>('private');
  ...
  const fetchContent = async () => {
    try {
        const { data } = await axios.get<ContentItem[]>(`${API_BASE}/contents`, {
  ...
  const checkPublicPrivate = async () => {
    try {
      const { data } = await axios.get<{ type: "public" | "private"; hash: string }>(`${API_BASE}/brain/public_OR_private`, {
        headers: { Authorization: token }
      })
  ...
  const deleteContent = async (id:string) => {
    try {
      await axios.delete(`${API_BASE}/deleteContent`, {
        headers: { Authorization: token },
        params: { id }
      })
      fetchContent()
                ...
                (item) =>
                  item.link && (
                    <Card
                      ...
                      title={item.title}
                      type={item.type as "Youtube" | "Twitter" | "Instagram" | "Notes" | "Link" | "Music"}
                      link={item.link}
